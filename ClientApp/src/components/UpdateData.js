...
export class UpdateData extends Component {
  ...
  componentDidMount() {
    axios
      ...
      .then(response => {
        this.setState({
          product_id: response.data.id_products,
          product_name: response.data.product_name,
          product_description: response.data.product_description,
          colors: response.data.colors,
          dataprice: response.data.dataprice,
          class1: response.data.class1,
          class2: response.data.class2,
  ...
  onSubmit(e) {
    ...
    axios
      .post(`api/Product/${this.state.product_id}`, product)
      .then(res => console.log(res.data));

    window.location = "/fetch-data";
  }
